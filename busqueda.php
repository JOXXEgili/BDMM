...
<body>
    ...
    <section class="cursos">
        <div class="CursosHeader">
            <span>Resultados para su búsqueda</span>
        </div>
        <?php

            $count = 0;

            if($_SESSION['busqueda1'] != null){
                $txt = $_SESSION['busqueda1'];
                $query = "SELECT ID_curso ,Nombre, Precio, Descrip, Miniatura, Puntuacion, Estado, Autor FROM cursos WHERE nombre LIKE '%$txt%' AND Estado = 1;";
                $resultado = $conn->query($query);
            }
            else if($_SESSION['busqueda2'] != null){
                $txt = $_SESSION['busqueda2'];
                $query = "SELECT * FROM categorias_cursos_info WHERE nombreCat = '$txt' AND Estado = 1";
                $resultado = $conn->query($query);
            }
            else if($_SESSION['busqueda3'] != null){
                $txt = $_SESSION['busqueda3'];
                $txt2 = $_SESSION['busqueda4'];
                $query = "SELECT ID_curso ,Nombre, Precio, Descrip, Miniatura, Puntuacion, Estado, Autor, Creacion FROM cursos WHERE Estado = 1 AND Creacion BETWEEN '$txt' AND '$txt2'";
                $resultado = $conn->query($query);
            }

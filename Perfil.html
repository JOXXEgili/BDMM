    ...
    <head>
        ...
        <script>
            function register(){
                ...
                gender = document.getElementById("photo-input").value;

                var rePass = /^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/;
                var reEmail = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/;
                var reName = /^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/;
                
                if(reName.test(name) && reName.test(FLastName) && reName.test(MLastName) && reEmail.test(email) && rePass.test(pass) && logo.files.length != 0 && birth){
        ...
        <header>
            ...
            <div class="right-nav">
                ...
                <div class="dropdown-categories-content-profile">
                    <il>
                        <li><button onclick="location.href='Perfil.html'">Perfil</button></li>
                        <li><button onclick="location.href='Kardex.html'">Mis cursos</button></li>
                        <li><button onclick="location.href='index.html'">Salir</button></li>
                    </il>
            </div>
        </header>
